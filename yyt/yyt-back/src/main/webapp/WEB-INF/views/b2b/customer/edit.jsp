			...
			<div class="layout-1">
				...
				<c:choose>
					<c:when test="${resultMap.record_tag eq '居民用户'}">
						<input name="recordTag" type="radio"  value="居民用户" checked="checked"/><span>居民用户</span>
						<input name="recordTag" type="radio"  value="公福用户"/><span>公福用户</span>
					</c:when>
					<c:when test="${resultMap.record_tag eq'公福用户'}">
						<input name="recordTag" type="radio"  value="居民用户" /><span>居民用户</span>
						<input name="recordTag" type="radio"  value="公福用户" checked="checked"/><span>居民用户</span>
					</c:when>
				</c:choose>
			</div>
			<div class="layout-1">
			...
			<div class="layout-1">
				...
				<select name="itemName" id="itemName" style="width: 200px">
					<c:forEach items="${communities}" var="item">
						<c:choose>
							<c:when test="${item.id eq resultMap.community_id}">
								<span>true</span>
								<option value="${item.itemName}" selected="selected">${item.itemName}</option>
							</c:when>
							<c:otherwise>
				...
				<input type="text" placeholder="详细地址" style="margin-left:15px;width: 300px" value="${resultMap.unit}"/>
			</div>
			<div class="layout-1">
