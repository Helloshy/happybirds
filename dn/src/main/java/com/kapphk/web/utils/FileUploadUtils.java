...
//				}
	...
	public static String uploadFile(byte[] bytes, String targetPath, 
		...
		if(!ValidateUtils.isEmpty(bytes) && !ValidateUtils.isBlank(type)){
			...
			if("0".equals(type)){//证件类，同意放在papers目录
				...
				visitePath = rootPath+"/"+targetPath+"/"+filename_s ;
				...
				visitePath = rootPath+"/"+targetPath+"/"+fileName ;
	 ...
	 * 共用拷贝文件
	 */
	public static String copyFiles(byte[] bytes,String targetPath,String type,HttpServletRequest request)throws Exception{
		String rootPath = "/uploadIMG" ;
		String path2 = FileUploadUtils.class.getClassLoader().getResource("../../").getPath() ;
		String contextPath = request.getContextPath() ;
		String path = path2.replace(contextPath, rootPath);
		String name = "/"+DateUtils.getLocalDate("yyyyMMddHHmmss") + DataUtils.getSixRadom()+type;
		path = path + targetPath;
		File file = new File(path);
		if(!file.exists()) file.mkdirs();
		...
		out.flush();
		out.close();
		return rootPath+"/"+targetPath+name;
	}
}
