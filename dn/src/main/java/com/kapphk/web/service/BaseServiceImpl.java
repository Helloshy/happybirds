...
public abstract class BaseServiceImpl<M, T>{
	 ...
	 * @throws Exception
	 */
	protected Integer saveOrUpdate(BaseMapper mapper,BaseModel bean) throws Exception
	{
		Integer r = 0;
		if(!ValidateUtils.isBlank(bean))
		{
			Method method = bean.getClass().getDeclaredMethod("getId", null);
			Object object = method.invoke(bean, null);
			
			if(!ValidateUtils.isBlank(object))
			{
				r = mapper.update(bean);
			}
			else
			{
				r = mapper.insert(bean);
			}
		}
		return r;
	}
