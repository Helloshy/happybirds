...
var save = function(){
	...
	if(state == 1 && $("#orderAmount").textbox("getValue") == '' || $("#orderAmount").textbox("getValue") == 0){
		$.messager.alert("警告","费用必填");
		return;
	}
...
var gridEdit = function(rowIndex){
	...
	var success = function(ret){
		if(ret && ret.status == 10001){
			...
			$("#teachTimes").textbox("setValue",data.teachTimes);
...
var state = function(state){
	...
	for(var i in sele){
		if(sele[i].state == 3){
			if(state ==1 || state == 2){
				$.messager.alert("提示","不能包含【取消】状态,请先筛选");
				return;
			}
		}
		ids.push(sele[i].id);
...
var exportExcel = function(type){
	window.location.href = dataOptions.baseUrl+'exportExcel.htm?recordType=1&type='+type+"&"+$('#search-form').serialize();
} ;
